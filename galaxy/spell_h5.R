...
if (is.null(opt$desc) | is.null(opt$model) | is.null(opt$inp) | is.null(opt$out) | is.null(opt$ids) | is.null(opt$ods)) {
	print_help(prase)
...
out[[opt$ods]] <- predict$out
out$close_all()

if (!is.null(opt$bad)) {
	drop=H5File$new(opt$bad,mode="w")
	drop[[opt$ods]] <- predict$dropped
	drop$close_all()
}
